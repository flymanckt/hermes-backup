...
_DEFAULT_FEISHU_CONNECT_ATTEMPTS = 5
_DEFAULT_FEISHU_SEND_ATTEMPTS = 5
_DEFAULT_FEISHU_API_TIMEOUT_SECONDS = 20.0
_DEFAULT_FEISHU_CONNECT_RETRY_BASE_SECONDS = 2.0
_DEFAULT_FEISHU_SEND_RETRY_BASE_SECONDS = 2.0
...
class FeishuAdapterSettings:
    ...
    connect_retry_base_seconds: float
    send_retry_base_seconds: float
    webhook_host: str
    webhook_port: int
    webhook_path: str
...
def _coerce_float(value: Any, default: Optional[float] = None, min_value: float = 0.0) -> Optional[float]:
    """Coerce value to float with optional default and minimum constraint."""
    try:
        parsed = float(value)
    except (TypeError, ValueError):
        return default
...
def _coerce_required_float(value: Any, default: float, min_value: float = 0.0) -> float:
    parsed = _coerce_float(value, default=default, min_value=min_value)
    return default if parsed is None else parsed
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            connect_attempts=_coerce_required_int(
                extra.get("connect_attempts") or os.getenv("HERMES_FEISHU_CONNECT_ATTEMPTS"),
                default=_DEFAULT_FEISHU_CONNECT_ATTEMPTS,
                min_value=1,
            ),
            send_attempts=_coerce_required_int(
                extra.get("send_attempts") or os.getenv("HERMES_FEISHU_SEND_ATTEMPTS"),
                default=_DEFAULT_FEISHU_SEND_ATTEMPTS,
                min_value=1,
            ),
            api_timeout_seconds=_coerce_required_float(
                extra.get("api_timeout_seconds") or os.getenv("HERMES_FEISHU_API_TIMEOUT_SECONDS"),
                default=_DEFAULT_FEISHU_API_TIMEOUT_SECONDS,
                min_value=1.0,
            ),
            connect_retry_base_seconds=_coerce_required_float(
                extra.get("connect_retry_base_seconds") or os.getenv("HERMES_FEISHU_CONNECT_RETRY_BASE_SECONDS"),
                default=_DEFAULT_FEISHU_CONNECT_RETRY_BASE_SECONDS,
                min_value=0.1,
            ),
            send_retry_base_seconds=_coerce_required_float(
                extra.get("send_retry_base_seconds") or os.getenv("HERMES_FEISHU_SEND_RETRY_BASE_SECONDS"),
                default=_DEFAULT_FEISHU_SEND_RETRY_BASE_SECONDS,
                min_value=0.1,
            ),
            webhook_host=str(
    ...
    def _apply_settings(self, settings: FeishuAdapterSettings) -> None:
        ...
        self._connect_attempts = settings.connect_attempts
        self._send_attempts = settings.send_attempts
        self._api_timeout_seconds = settings.api_timeout_seconds
        self._connect_retry_base_seconds = settings.connect_retry_base_seconds
        self._send_retry_base_seconds = settings.send_retry_base_seconds
    ...
    async def edit_message(
        ...
        try:
            ...
            response = await self._update_message_with_retry(request, message_id=message_id)
            ...
            if not result.success and msg_type == "post" and _POST_CONTENT_INVALID_RE.search(result.error or ""):
                ...
                fallback_response = await self._update_message_with_retry(
                    fallback_request,
    ...
    async def _update_approval_card(
        ...
        try:
            ...
            request = self._build_update_message_request(message_id=message_id, request_body=body)
            await self._update_message_with_retry(request, message_id=message_id)
        except Exception as exc:
    ...
    async def send_image_file(
        ...
        **kwargs,
        ...
        try:
            ...
            upload_response = await self._run_sdk_call(
                self._client.im.v1.image.create,
                request,
                op_name="upload_image",
            )
    ...
    async def _handle_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        try:
            ...
            response = await self._run_sdk_call(
                ...
                request,
                op_name="get_message_for_reaction",
            )
    ...
    async def _download_feishu_image(self, *, message_id: str, image_key: str) -> tuple[str, str]:
        ...
        try:
            ...
            response = await self._run_sdk_call(
                ...
                request,
                op_name="download_image_resource",
            )
        ...
        for request_type in request_types:
            try:
                ...
                response = await self._run_sdk_call(
                    self._client.im.v1.message_resource.get,
                    request,
                    op_name=f"download_{request_type}_resource",
                )
                if not response or not response.success():
                    logger.debug(
    ...
    async def _hydrate_bot_identity(self) -> None:
        ...
        try:
            ...
            response = await self._run_sdk_call(
                self._client.application.v6.application.get,
                request,
                op_name="get_application",
            )
            if not response or not response.success():
        ...
        for attempt in range(self._connect_attempts):
            try:
                ...
                if attempt >= self._connect_attempts - 1:
                    raise
                wait_seconds = self._retry_backoff_seconds(
                    base_seconds=self._connect_retry_base_seconds,
                    attempt=attempt,
                    max_seconds=30.0,
                )
                logger.warning(
                    "[Feishu] Connect attempt %d/%d failed; retrying in %.1fs: %s",
                    attempt + 1,
        ...
        for attempt in range(self._send_attempts):
            try:
                ...
                if attempt >= self._send_attempts - 1:
                    raise
                wait_seconds = self._retry_backoff_seconds(
                    base_seconds=self._send_retry_base_seconds,
                    attempt=attempt,
                    max_seconds=20.0,
                )
                logger.warning(
                    "[Feishu] Send attempt %d/%d failed for chat %s; retrying in %.1fs: %s",
                    attempt + 1,
                    self._send_attempts,
                    chat_id,
                    wait_seconds,
    ...
    async def _update_message_with_retry(self, request: Any, *, message_id: str) -> Any:
        """Retry transient message update failures so streaming/edit responses degrade less often."""
        last_error: Optional[Exception] = None
        for attempt in range(self._send_attempts):
            try:
                return await self._run_sdk_call(
                ...
                logger.warning(
                    "[Feishu] Update attempt %d/%d failed for message %s; retrying in %.1fs: %s",
                    attempt + 1,
                    self._send_attempts,
                    message_id,
                    wait_seconds,
                ...
                await asyncio.sleep(wait_seconds)
        raise last_error or RuntimeError("Feishu update failed")

    @staticmethod
    def _retry_backoff_seconds(*, base_seconds: float, attempt: int, max_seconds: float) -> float:
        """Bounded exponential backoff for transient Feishu transport failures."""
        return min(base_seconds * (2 ** max(attempt, 0)), max_seconds)

    async def _run_sdk_call(self, func: Any, *args: Any, op_name: str) -> Any:
        """Run a blocking lark-oapi SDK call with a bounded timeout."""
        try:
            ...
            raise TimeoutError(
                f"[Feishu] {op_name} timed out after {self._api_timeout_seconds:.1f}s"
            ) from exc

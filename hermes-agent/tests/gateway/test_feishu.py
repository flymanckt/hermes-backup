...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_feishu_send_retries_honour_configured_attempts(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter

        adapter = FeishuAdapter(
            PlatformConfig(extra={"send_attempts": 4, "send_retry_base_seconds": 0.01})
        )
        adapter._client = SimpleNamespace(im=SimpleNamespace(v1=SimpleNamespace(message=SimpleNamespace())))

        attempts = {"count": 0}

        async def _failing_send_raw_message(**_kwargs):
            attempts["count"] += 1
        ...
        with (
            patch.object(adapter, "_send_raw_message", side_effect=_failing_send_raw_message),
            patch("gateway.platforms.feishu.asyncio.sleep", side_effect=_skip_sleep),
        ):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_run_sdk_call_times_out(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter

        adapter = FeishuAdapter(PlatformConfig(extra={"api_timeout_seconds": 1.0}))

        async def _raise_timeout(awaitable, *_args, **_kwargs):
            awaitable.close()
            raise asyncio.TimeoutError()

        with patch("gateway.platforms.feishu.asyncio.wait_for", side_effect=_raise_timeout):
            with self.assertRaises(TimeoutError):
                asyncio.run(adapter._run_sdk_call(lambda: None, op_name="create_message"))
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_edit_message_retries_transient_update_failures(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        adapter._client = SimpleNamespace(
            im=SimpleNamespace(
                v1=SimpleNamespace(
                    message=SimpleNamespace(update=Mock()),
                )
            )
        )
        ...
        with (
            patch.object(adapter, "_run_sdk_call", side_effect=responses) as mocked_call,
            patch("gateway.platforms.feishu.asyncio.sleep", side_effect=_skip_sleep),
        ):
            result = asyncio.run(
                adapter.edit_message(

...
class TestNonStringContent:
    ...
    def test_summary_prompt_preserves_branch_labels_and_aliases(self):
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "ok"
        ...
        messages = [
            {"role": "user", "content": "走B方案，A方案放弃，文件叫plan_b.md"},
            {"role": "assistant", "content": "收到，我会按B方案继续。"},
        ]
        ...
        assert "Conversation Anchors" in prompt
        assert "A/B/C options" in prompt
        assert "Do NOT collapse named alternatives into generic prose" in prompt

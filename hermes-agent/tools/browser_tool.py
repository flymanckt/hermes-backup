...
BROWSER_TOOL_SCHEMAS = [
    ...
    {
        "name": "browser_upload",
        "description": "Upload one or more local files into a file input identified by its ref ID (for example resume uploads on job sites). Paths must exist on the current machine. Requires browser_navigate and browser_snapshot to be called first.",
        "parameters": {
            "type": "object",
            "properties": {
                "ref": {
                    "type": "string",
                    "description": "The file input reference from the snapshot (e.g., '@e7')"
                },
                "paths": {
                    "type": "array",
                    "items": {"type": "string"},
                    "minItems": 1,
                    "description": "One or more local file paths to upload. Example: ['/mnt/c/Users/Kent/Documents/resume.pdf']"
                }
            },
            "required": ["ref", "paths"]
        }
...
def browser_upload(ref: str, paths: List[str], task_id: Optional[str] = None) -> str:
    """Upload one or more local files into a file input element."""
    if _is_camofox_mode():
        return json.dumps({
            "success": False,
            "error": "browser_upload is not supported in Camofox mode yet. Disable CAMOFOX_URL to use the standard browser backend."
        }, ensure_ascii=False)
    ...
    if not isinstance(paths, list) or not paths:
        return json.dumps({
            "success": False,
            "error": "Provide at least one file path in 'paths'."
        }, ensure_ascii=False)
    ...
    for raw_path in paths:
        ...
        if not candidate:
            return json.dumps({
                "success": False,
                "error": "File paths must be non-empty strings."
            }, ensure_ascii=False)
        ...
        if not resolved.exists():
            return json.dumps({
                "success": False,
                "error": f"File does not exist: {candidate}"
            }, ensure_ascii=False)
        if not resolved.is_file():
            return json.dumps({
                "success": False,
                "error": f"Upload target must be a file, not a directory: {candidate}"
            }, ensure_ascii=False)
        resolved_paths.append(str(resolved))

    result = _run_browser_command(effective_task_id, "upload", [ref] + resolved_paths)

    if result.get("success"):
        uploaded = result.get("data", {}).get("uploaded", resolved_paths)
        return json.dumps({
            "success": True,
            "element": ref,
    ...
    return json.dumps({
        "success": False,
        "error": result.get("error", f"Failed to upload files into {ref}")
    }, ensure_ascii=False)

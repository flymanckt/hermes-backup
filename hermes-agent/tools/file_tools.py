...
_LEGACY_RUNTIME_READ_OVERRIDE_ENV = "HERMES_ALLOW_LEGACY_RUNTIME_READS"
_LEGACY_RUNTIME_HOME_DIRS = (".openclaw", ".clawdbot", ".moltbot")
...
def _legacy_runtime_read_roots() -> tuple[Path, ...]:
    """Return legacy runtime roots that should not be used as Hermes context."""
    home = Path.home()
    return tuple((home / dirname).resolve(strict=False) for dirname in _LEGACY_RUNTIME_HOME_DIRS)


def _check_legacy_runtime_read_path(path: str) -> str | None:
    """Block accidental reads/searches against legacy OpenClaw-era runtime dirs."""
    if _is_truthy_env(_LEGACY_RUNTIME_READ_OVERRIDE_ENV):
        return None
    ...
    for blocked_root in _legacy_runtime_read_roots():
        try:
            resolved.relative_to(blocked_root)
            return (
                f"Access denied: {path} points into legacy runtime state under {blocked_root}. "
                "Hermes should use ~/.hermes state instead of OpenClaw-era files for context. "
                f"If you are intentionally doing a migration/debug read, set {_LEGACY_RUNTIME_READ_OVERRIDE_ENV}=1."
            )
        except ValueError:
            continue
    return None
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        legacy_runtime_err = _check_legacy_runtime_read_path(path)
        if legacy_runtime_err:
            return json.dumps({"error": legacy_runtime_err}, ensure_ascii=False)

...
[Preserve exact labels the user may reuse later: A/B/C options, plan names, branch names, aliases, shorthand references, IDs, exact quoted phrases, variable names, filenames, ticket numbers, and dates. If the user said "use plan B" or "go with option A", keep the exact mapping.]
...
Do NOT collapse named alternatives into generic prose. Preserve exact branch/option labels and what each label means.
If there are competing plans, retain the final selected one and the rejected alternatives by name.
